...
export const getAllItems = catchAsyncErrors(async (req, res, next) => {
  ...
  const formattedItems = items.map((item) => ({
    ...item._doc,
    startTime: new Date(item.startTime).toLocaleString("en-US", {
      timeZone: "America/New_York",
    }),
    endTime: new Date(item.endTime).toLocaleString("en-US", {
      timeZone: "America/New_York",
    }),
  }));
  res.status(200).json({
...
export const getAuctionDetails = catchAsyncErrors(async (req, res, next) => {
  ...
  res.status(200).json({
    ...
    auctionItem: {
      ...auctionItem._doc,
      startTime: new Date(auctionItem.startTime).toLocaleString("en-US", {
        timeZone: "America/New_York",
      }),
      endTime: new Date(auctionItem.endTime).toLocaleString("en-US", {
        timeZone: "America/New_York",
      }),
    },
    bidders,
